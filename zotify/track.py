...
def parse_track_metadata(track_resp: dict) -> dict[str, list[str] | str | int | bool]:
    ...
    track_metadata[ARTISTS] = [
        artist[NAME] for artist in track_resp[ARTISTS]
    ]  # list[str]
    track_metadata[ARTIST_IDS] = [
        artist[ID] for artist in track_resp[ARTISTS]
    ]  # list[str]
    ...
    track_metadata[TOTAL_TRACKS] = str(track_resp[ALBUM][TOTAL_TRACKS]).zfill(
        2
    )  # int as str
    ...
    track_metadata[COMPILATION] = (
        1 if COMPILATION in track_resp[ALBUM][ALBUM_TYPE] else 0
    )  # int
    track_metadata[DURATION_MS] = track_resp[DURATION_MS]  # int

    largest_image = max(
        track_resp[ALBUM][IMAGES], key=lambda img: img[WIDTH], default=None
    )
    ...
    track_metadata[IS_PLAYABLE] = (
        track_resp[IS_PLAYABLE] if IS_PLAYABLE in track_resp else True
    )  # bool, assume true
...
def get_track_lyrics(track_id: str) -> list[str]:
    ...
    (raw, lyrics_dict) = Zotify.invoke_url(
        "https://spclient.wg.spot" + f"ify.com/color-lyrics/v2/track/{track_id}",
        expectFail=True,
    )
    if lyrics_dict:
        try:
...
def handle_lyrics(
    track_id: str, filedir: PurePath, track_metadata: dict
) -> list[str] | None:
    lyrics = None
    if (
        not Zotify.CONFIG.get_download_lyrics()
        and not Zotify.CONFIG.get_always_check_lyrics()
    ):
        return lyrics
...
def download_track(
    ...
    if Zotify.CONFIG.get_download_parent_album():
        if (
            ...
            try:
                (raw, info) = Zotify.invoke_url(
                    f"{TRACK_URL}?ids={track_id}&market=from_token"
                )
    ...
    try:
        ...
        with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."):
            ...
            if Zotify.CONFIG.get_temp_download_dir() != "":
                track_path_temp = PurePath(
                    Zotify.CONFIG.get_temp_download_dir()
                ).joinpath(f"zotify_{str(uuid.uuid4())}_{track_id}.{track_path.suffix}")

            track_path_exists = (
                Path(track_path).is_file() and Path(track_path).stat().st_size
            )
            ...
            if (
                ...
                c = len(
                    [
                        ...
                        if file.match(track_path.stem + "*")
                    ]
                )
                track_path = PurePath(filedir).joinpath(
                    f"{track_path.stem}_{c}{track_path.suffix}"
                )
                track_path_exists = False  # new track_path guaranteed to be unique

            liked_m3u8 = (
                child_request_mode == "liked"
                and Zotify.CONFIG.get_liked_songs_archive_m3u8()
            )
            if Zotify.CONFIG.get_export_m3u8() and track_id == child_request_id:
                m3u8_path: PurePath | None = (
                    extra_keys["m3u8_path"] if "m3u8_path" in extra_keys else None
                )
                if liked_m3u8:
                ...
                if liked_m3u8:
                    if songs_m3u is not None and track_m3u8_label in songs_m3u[0]:
                        ...
                        Path(
                            filedir / (Zotify.DATETIME_LAUNCH + "_zotify.m3u8")
                        ).replace(m3u8_path)
        ...
        try:
            if not track_metadata[IS_PLAYABLE]:
                Printer.hashtaged(
                    PrintChannel.SKIPPING, f'"{track_label}" (TRACK IS UNAVAILABLE)'
                )
            else:
                if (
                    track_path_exists
                    ...
                    Printer.hashtaged(
                        PrintChannel.SKIPPING, f'"{track_label}" (TRACK ALREADY EXISTS)'
                    )

                elif (
                    in_global_songids and Zotify.CONFIG.get_skip_previously_downloaded()
                ):
                    Printer.hashtaged(
                        PrintChannel.SKIPPING,
                        f'"{track_label}" (TRACK ALREADY DOWNLOADED ONCE)',
                    )
                    ...
                    try:
                        set_audio_tags(
                            track_path_temp, track_metadata, total_discs, genres, lyrics
                        )
                        set_music_thumbnail(
                            track_path_temp, track_metadata[IMAGE_URL], mode
                        )
                    except Exception as e:
                        Printer.hashtaged(
                            PrintChannel.ERROR,
...
def convert_audio_format(track_path) -> None:
    ...
    try:
        ...
        Printer.hashtaged(
            ...
            reason + f"SKIPPING CONVERSION TO {file_codec.upper()}",
        )
