...
from zotify.const import TRACKS, ALBUM, GENRES, NAME, DISC_NUMBER, TRACK_NUMBER, TOTAL_TRACKS, \
    IS_PLAYABLE, IMAGES, URL, RELEASE_DATE, ID, TRACKS_URL, TRACK_STATS_URL, \
...
from zotify.utils import fix_filename, set_audio_tags, set_music_thumbnail, create_download_directory, \
    add_to_m3u8, fetch_m3u8_songs, add_to_directory_song_archive, \
...
def download_track(mode: str, track_id: str, extra_keys: dict | None = None, pbar_stack: list | None = None) -> None:
    ...
    try:
        ...
        # Proper duplication check logic
        is_duplicate = False
        if Path(filename).is_file() and Path(filename).stat().st_size > 0:
            try:
                # If file exists, check metadata
                existing_tags = music_tag.load_file(filename)
                if (existing_tags['tracktitle'] == name and
                        existing_tags['artist'] == artists[0]):
                    is_duplicate = True
                    Printer.debug("Duplicate Check: Found existing file with matching metadata.")
            except Exception as e:
                Printer.debug(f"Could not read metadata from existing file: {e}")

        check_all_time = scraped_song_id in get_archived_song_ids()
        Printer.debug("Duplicate Check\n" + \
                      f"File is Duplicate (Metadata Check): {is_duplicate}\n" + \
        ...
        try:
            if not is_playable:
                ...
                if is_duplicate and Zotify.CONFIG.get_skip_existing():
                    prepare_download_loader.stop()
                    Printer.print(PrintChannel.SKIPS,
                                  f'###   SKIPPING:  "{song_name}" (TRACK ALREADY EXISTS WITH MATCHING METADATA)   ###')

                elif check_all_time and Zotify.CONFIG.get_skip_previously_downloaded():
                    prepare_download_loader.stop()
                    Printer.print(PrintChannel.SKIPS,
                                  f'###   SKIPPING:  "{song_name}" (TRACK ALREADY DOWNLOADED ONCE)   ###')
                    ...
                    Printer.print(PrintChannel.DOWNLOADS,
                                  f'###   DOWNLOADED: "{Path(filename).relative_to(Zotify.CONFIG.get_root_path())}"   ###\n' + \
                                  f'###   DOWNLOAD TOOK {time_elapsed_dl} (PLUS {time_elapsed_ffmpeg} CONVERTING)   ###')

                    # add song ID to global .song_archive file, now that we know its not a duplicate
                    # Note: You may want to rethink this logic if you want to avoid re-downloading things that were skipped due to metadata check
                    if not check_all_time:
                        add_to_song_archive(scraped_song_id, PurePath(filename).name, artists[0], name)

                    # Original logic for local archive check was removed, so this will always run if not a metadata duplicate
