...
def add_to_directory_song_archive(download_path: str, song_id: str, filename: str, author_name: str,
                                  song_name: str) -> None:
...
def set_audio_tags(filename, artists: list[str], genres: list[str], name, album_name, album_artist, release_year,
                   disc_number, track_number, total_tracks, total_discs, compilation: int,
...
def fix_filename(name: str | PurePath | Path):
    ...
    True
    """
    name = re.sub(r'[/\\:|<>"?*\0-\x1f]|^(AUX|COM[1-9]|CON|LPT[1-9]|NUL|PRN)(?![^.])|^\s|[\s.]$', "_", str(name),
                  flags=re.IGNORECASE)
