...
def download_episode(episode_id, pbar_stack: list | None = None) -> None:
    ...
    with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."):
        ...
        episode_path = (
            PurePath(Zotify.CONFIG.get_root_podcast_path())
            / podcast_name
            / f"{filename}.tmp"
        )
        ...
        if (
            "anon-podcast.scdn.co" in direct_download_url
            or "audio_preview_url" not in resp
        ):
            ...
            episode_path_exists = (
                Path(episode_path).is_file()
                and Path(episode_path).stat().st_size == total_size
            )
