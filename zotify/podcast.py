...
from zotify.const import EPISODE_INFO_URL, SHOWS_URL, PARTNER_URL, PERSISTED_QUERY, ERROR, ID, ITEMS, NAME, SHOW, \
    DURATION_MS
        ...
        if "anon-podcast.scdn.co" in direct_download_url or "audio_preview_url" not in resp:
            ...
            if stream is None:
                Printer.print(PrintChannel.ERRORS,
                              '###   ERROR:  SKIPPING EPISODE - FAILED TO GET CONTENT STREAM   ###\n' + \
                              f'###   Episode_ID: {str(episode_id)}   ###')
                ...
                if (Path(filepath).is_file()
                    ...
                    Printer.print(PrintChannel.SKIPS,
                                  f'###   SKIPPING:  "{podcast_name} - {episode_name}" (EPISODE ALREADY EXISTS)   ###')
                    return
                prepare_download_loader.stop()
